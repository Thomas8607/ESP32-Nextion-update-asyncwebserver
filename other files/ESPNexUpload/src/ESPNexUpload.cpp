    ...
    for(uint16_t i = 0; i < buf_size; i++) {
		...
		if(_sent_packets == 4096){
			...
			if(string.indexOf(0x05) != -1) { 
				_sent_packets = 0; // reset sent packets counter
				string = ""; // reset receive String
			}
			else {
				if (timeout >= 8) {
			...
			i--; // delay current byte
		}
		else {
			c = file_buf[i]; // read buffer
			nexSerial.write(c); // write byte to nextion over serial
			_sent_packets++; // update sent packets counter
		}
    }
    return true;  
}
